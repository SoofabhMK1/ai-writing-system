...
class OutlineNode(Base):
    ...
    generated_content = Column(Text, nullable=True, comment="AI生成或用户最终编辑的内容")
    word_count_target = Column(Integer, default=0, comment="目标字数")
    status = Column(Enum(NodeStatus), default=NodeStatus.TODO, nullable=False, comment="节点状态")
    
    project_id = Column(Integer, ForeignKey("projects.id"), nullable=False, index=True)
    parent_id = Column(Integer, ForeignKey("outline_nodes.id"), nullable=True, index=True)
