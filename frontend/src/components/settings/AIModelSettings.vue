    ...
    <div v-if="aiModels.length > 0" class="setting-item-list">
      <div v-for="model in aiModels" :key="model.id" class="setting-item">
        ...
        <div class="item-actions">
          <button @click="handleTest(model)" class="btn btn-secondary">
            测试
          </button>
          <button @click="handleEdit(model)" class="btn">编辑</button>
          <button @click="handleDelete(model.id)" class="btn btn-danger">
            删除
          </button>
        </div>
      </div>
...
const modalTitle = computed(() =>
  isEditing.value ? '编辑 AI 模型' : '添加新 AI 模型',
)
...
const saveModel = async (data) => {
  try {
    if (isEditing.value) {
      ...
      const index = aiModels.value.findIndex(
        (m) => m.id === currentModel.value.id,
      )
      if (index !== -1) {
...
const handleTest = async (model) => {
  ...
  try {
    ...
    notification.show(`${model.name} 测试失败: ${detail}`, 'error', {
      duration: 5000,
    })
  }
}
